...
void move_forward(oi_t *sensor_data, int centimeters) // working
{
    ...
    while (sum < ((centimeters - 1) * 10) - 7)
    {
                ...
                || sensor_data->cliffFrontLeft || sensor_data->cliffFrontRight)
        {
            ...
            sprintf(line,
                    " Crater State: cliffleft:%d Cliffright:%d cliffFrontLeft:%d cliffFrontRight:%d\r\n",
        ...
        if (sensor_data->cliffLeftSignal > 2800
                || sensor_data->cliffFrontLeftSignal > 2800
                || sensor_data->cliffFrontRightSignal > 2800
                || sensor_data->cliffRightSignal > 2800)
        {
            if (sensor_data->cliffLeftSignal > 2800)
            {
                backward(sensor_data, 100);
            ...
            sprintf(line,
                    " Boundary State: WhiteLeft:%d WhiteRight:%d WhiteFrontLeft:%d WhiteFrontRight:%d\r\n",
                    whiteleftSig, whiterightSig, whitefrontLeftSig,
                    whitefrontRightSig);
            uart_sendStr(line);
            break;
        }
...
void main()
{
    ...
    while (1)
    {
        ...
        else if (strcmp(splitStrings[0], "ms") == 0) //move servo
        {
            moveServo(dec, 1);
            for (i = 0; i < 2; i++)
                for (j = 0; j < 30; j++)
        ...
        else if (strcmp(splitStrings[0], "sonar") == 0) //sonar test
        {
            sonar_test();
            for (i = 0; i < 2; i++)
                for (j = 0; j < 30; j++)
